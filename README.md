...
- `ADMIN_EMAILS` — email-адреса администраторов через запятую (например `admin@mail.com,owner@mail.com`)
- `OWNER_EMAIL` — главный администратор (владелец), который может назначать/снимать админов и удалять админов
...
- Страница: `admin.html` (доступ только для роли `admin`).
- Панель умеет:
   - управлять пользователями (роль, блокировка, удаление),
   - добавлять/удалять новости,
   - редактировать список топ-игроков.

Права владельца:
- только `OWNER_EMAIL` может назначать и снимать администраторов,
- только `OWNER_EMAIL` может удалять администраторов,
- аккаунт владельца нельзя удалить или заблокировать.

Чтобы сделать пользователя админом:
- добавь его email в `ADMIN_EMAILS` в `backend/.env`,
- перезапусти backend,
- заново войди под этим пользователем.
